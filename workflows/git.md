...
이 워크플로우는 사용자가 `/git` 커맨드를 입력했을 때, 로컬 변경 사항을 검사·스테이징하고 시맨틱 커밋 메시지(feat, fix, docs 등)를 자동 생성하여 `origin main` 브랜치에 푸시한 후, 선택적으로 GCP 배포(`/gcp`)까지 연속 실행하기 위한 에이전트 지침입니다.
